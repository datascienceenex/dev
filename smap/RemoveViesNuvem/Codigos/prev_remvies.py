...
Script para atualizar os arquivos.csv usados na remoção de vies
dentro da pasta Traballho/<subbacia>/<nome_modelo>.csv


Script for creating  PMEDIA and also for preparing before de 'Remove Vies ''



python prev_remvies.py <modelo>  <nprev> 

modelo: string. Exemplos:

modelo=['TOKA','ECMWF','GFS','TOK30',]
...
print('Running prev_remvies for %s........'%MODELO)
...
for file in FILES:
    ...
    print('reading file %s and processing ......' %file)
    print(date_i,date_p)
...
for var,Data in ALL_LIST.groupby(['subb','bacia']):
    ...
    if not os.path.exists(FILE_TRABALHO):
        ...
        print('The file %s does not exist, will be created ' %FILE_TRABALHO)
        ...
        if sum(DDF==1)==len(DDF) and 1 in DIF_dias: # condiço para saber si os dados baixados eles estao cada 1 dia e existe o dia que continua o ultimo dia
            for nidx,idx in enumerate(DATA.index):
                DIFF=(idx-DATA_C.index[-1]).days 
                if DIFF>0:
                    P+=1
                    print('Appending date %s  to %s' %(idx.date(),FILE_TRABALHO))
            ...
            if P==0:
                print('Does not appending to  data found in %s' %(FILE_TRABALHO))
        else:
            print(' ...warning ...\n Appending does not make to %s .....\n because modeleled days do not continue the last date or files in %s are not equally 1-day spaced \n' %(FILE_TRABALHO,'Arq_Entrada/Previsao/%s' %MODELO))
